...
class VfTwo():
    ...
    def __init__(self, g, h):
        ...
        self.core_s_g = dict.fromkeys(self.small_g.nodes, self.null_n)
        self.core_l_g = dict.fromkeys(self.large_g.nodes, self.null_n)
        ...
        self.t_in_s = self.t_out_s = dict.fromkeys(self.small_g.nodes, 0)
        self.t_in_l = self.t_out_l = dict.fromkeys(self.large_g.nodes, 0)
    ...
    def match( self, depth=0, last_mapped=(Node("0", ""), Node("0", "")), ):
        ...
        for tup in p:
            if self.feasable(tup[0], tup[1], depth):
                self.compute_state()
                self.match(core_s_g, depth + 1, tup)
        self.restore_ds()
    ...
    def matching_covers_g2(self):
        ...
        for vert in self.core_s_g:
            if not vert in self.core_l_g:
                return False
        return True

    def compute_p(self):
        ''' computes the canditates p by the rules defined in the paper'''
        my_vals = [self.t_len[x] for x in self.t_len]  # list of vals in dict
        ...
        #  checks if both t_outs are empty and both t_ins are note empty
        elif not any(my_vals[2], my_vals[4]) and all(my_vals[1], my_vals[3]):
            ...
            return self.cart_prod(diff_l, min(diff_s))
    ...
    def compute_state ( self,tupel ):
        ...
        the cores
        '''
        self.core_l_g[ tupel[0] ] = tuperl[1]
